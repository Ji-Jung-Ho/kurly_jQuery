(($)=>{
    const SignUp = {
          ...
          $idOkBtn.on({
            click: function(e){
                ...
                if( regExp2.test( thisVal ) === false || regExp3.test(thisVal) === false || regExp4.test(thisVal) === true ){
                  ...
                  $.ajax({
                    url: 'https://kiik52.com/kurly_study/member_select.php', // 조회(검색) PHP
                    type: "GET",
        ...
        pwMethod(){ 
          ...
          $inputPw1.on({
            keyup(){
              ...
              // 2. 영문/숫자/특수문자(공백 제외)만 허용하며, 2개 이상 조합, 영문/숫자 조합 또는 영문/특수문자 또는 숫자/특수문자
              // 3. 공백제외
            ...
            if($inputEmail.val()===''){
              ...
              $.ajax({
                ...
                success: function(result){
                    ...
                    try {
                        let imsi = JSON.parse(result);
                        let res = imsi.map((item)=>item.이메일===$inputEmail.val());
                        if( res.includes(true) ){
                          $('#confirmModal').fadeIn(300);
        ...
        hpMethod(){
          ...
          $('#inputHp').on({
            ...
            $('.hp-num-btn').on({
              click(e){
                e.preventDefault();
                // 앞에는 무조건 01[1,2,6,7,8,9]로 시작하고 + 3~4자리수의 0~9까지의 수 + 4자리수의 0~9까지의 수
                ...
                if( regExp2.test( $('#inputHp').val() )===false ){
                  ...
                  $('#confirmModal h2').html(`인증번호가 발송되었습니다. <p style='font-size:18px'>${num}</p>`);
        ...
        addrMethod() {
          // 6. 주소 메서드
          const that = this;
          // 1. 주소 검색 자식창(윈도우 팝업창) 만들기
          // 2. 자식창(윈도우 팝업창)에 주소1, 주소2 입력 상자 및 UIUX 디자인 제작
            ...
            click(e){
              ...
              $.ajax({
                ...
                data: {
                  ...
                  gender : 회원가입데이터.성별,
                  birth : 회원가입데이터.생년월일,
                  addInput : 회원가입데이터.추가입력사항,
                  service : JSON.stringify(회원가입데이터.이용약관동의),
                  gaibDate : 회원가입데이터.가입일자,
                },
